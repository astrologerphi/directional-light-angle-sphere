...
const cylinderRadius = 1.4; // Radius of each circular cross-section (like torus minor radius)
const cylinderHeight = 3.0; // Total height representing 0–24h time axis
...
const numCrossSections = 24; // One cross-section per hour
const crossSectionSegments = 32; // Points per cross-section circle
const numVerticalLines = 8; // Vertical connecting lines

// Create a cylinder wireframe: circular cross-sections (like plane views stacked by time)
// plus vertical lines connecting them — an "unrolled torus"
function createCylinderWireframe(
    ...
    crossSectionVerts: number;
    verticalLineCount: number;
    verticalLineVerts: number;
    outlineVerts: number;
} {
    const vertices: number[] = [];
    const halfHeight = height / 2;

    // Draw cross-section circles at each hour mark (0h at bottom, 24h at top)
    for (let s = 0; s < numCrossSections; s++) {
        const y = -halfHeight + (s / (numCrossSections - 1)) * height;
        for (let i = 0; i <= crossSectionSegments; i++) {
            const angle = (i / crossSectionSegments) * Math.PI * 2;
    ...
    for (let i = 0; i < numVerticalLines; i++) {
        ...
        for (let s = 0; s <= verticalSteps; s++) {
            const y = -halfHeight + (s / verticalSteps) * height;
            vertices.push(x, y, z);
        }
    }

    // Top and bottom outline circles (denser)
    for (const y of [-halfHeight, halfHeight]) {
        for (let i = 0; i <= crossSectionSegments * 2; i++) {
            const angle = (i / (crossSectionSegments * 2)) * Math.PI * 2;
            const x = radius * Math.cos(angle);
            const z = radius * Math.sin(angle);
            vertices.push(x, y, z);
        }
    ...
    return {
        ...
        crossSectionCount: numCrossSections,
        crossSectionVerts: crossSectionSegments + 1,
        verticalLineCount: numVerticalLines,
        verticalLineVerts: verticalSteps + 1,
        outlineVerts: crossSectionSegments * 2 + 1,
    };
}

// Project 3D direction onto the cylinder at a given time.
// Time (0–24h) maps to Y height. The direction is stereographically projected
// onto the circular cross-section at that height (same as the plane view).
function projectToCylinder(time: number, direction: Vector3, radius = cylinderRadius, height = cylinderHeight): Vec3 {
    const halfHeight = height / 2;

    // Time -> Y position (0h at bottom, 24h at top)
    const y = -halfHeight + (time / 24) * height;

    // Stereographic projection of direction onto 2D (same as plane view)
    const scale = 1 / (1 + direction.y + 0.01);
    const projX = direction.x * scale;
    const projZ = direction.z * scale;

    // Map the 2D projection into the circular cross-section
    const distance = Math.sqrt(projX * projX + projZ * projZ);
    const angle = Math.atan2(projZ, projX);

    // Clamp distance to fit within the cross-section
    const r = Math.min(distance, 1.0) * radius * 0.9;
...
export async function initWebGPUVisualizationCylinder(
    ...
    // Create trail points array for each segment (3D positions on cylinder)
    const segmentTrails: { points: { position: Vec3; timestamp: number }[] }[] = segments.map(() => ({ points: [] }));

    const cylinder = createCylinderWireframe(cylinderRadius, cylinderHeight);
    const cylinderBuffer = createStaticBuffer(device, cylinder.vertices, GPUBufferUsage.VERTEX);
    ...
    // Horizontal rotation state
    const cameraDistance = 4.5;
    const cameraElevation = 2.0;
    let cameraAngle = Math.PI / 4; // Initial horizontal angle
    ...
    // Wireframe segment counts for draw calls
    const { crossSectionCount, crossSectionVerts, verticalLineCount, verticalLineVerts, outlineVerts } = cylinder;
    ...
    const render = (now?: number) => {
        ...
        // Project to cylinder surface (time -> height, direction -> cross-section)
        segments.forEach((_, idx) => {
            const trail = segmentTrails[idx];
            const direction = currentDirections[idx];
            const projected = projectToCylinder(cycleTime, direction);
        ...
        // Camera orbits horizontally around the cylinder
        const eyeX = Math.cos(cameraAngle) * cameraDistance;
        const eyeZ = Math.sin(cameraAngle) * cameraDistance;
        const viewMatrix = createViewMatrix([eyeX, cameraElevation, eyeZ], [0, 0, 0], [0, 1, 0]);
        ...
        segments.forEach((segment, idx) => {
            ...
            dotUniformData[16] = projected[0];
            dotUniformData[17] = projected[1];
            dotUniformData[18] = projected[2];
        ...
        // Draw cylinder wireframe grid
        passEncoder.setPipeline(linesPipeline);
        passEncoder.setBindGroup(0, bindGroup);
        passEncoder.setVertexBuffer(0, cylinderBuffer);

        let offset = 0;

        // Draw cross-section circles (one per hour)
        for (let i = 0; i < crossSectionCount; i++) {
            passEncoder.draw(crossSectionVerts, 1, offset, 0);
            offset += crossSectionVerts;
        }

        // Draw vertical connecting lines
        for (let i = 0; i < verticalLineCount; i++) {
            passEncoder.draw(verticalLineVerts, 1, offset, 0);
            offset += verticalLineVerts;
        }

        // Draw top and bottom outline circles
        for (let i = 0; i < 2; i++) {
            passEncoder.draw(outlineVerts, 1, offset, 0);
            offset += outlineVerts;
        }
    ...
    updateStatus('WebGPU Cylinder View ready. Playing animation.');
    animationFrameId = requestAnimationFrame(render);
